trigger IntegranteTrigger on Integrante__c (
	before insert, before update,
	after update
	)  {
	if (Trigger.isBefore) {
		if (Trigger.isInsert) {
			IntegranteHelper.checkIfTitularAlreadyHasFamiliarWithRepeatedDNI(Trigger.new);
			IntegranteHelper.avoidRepeatedDNIsInTheOpportunity(Trigger.new);
