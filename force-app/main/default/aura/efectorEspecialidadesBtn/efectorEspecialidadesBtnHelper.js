({
    goToApex: function (component, event, codEfector) {
        ...
        LightningUtils.callApex(
            component,
            "getEspecialidadesEfector",
            function(succeed, result, errors) {
                if(succeed) {
                    if(!result.hasError) {
                        if(result.especialidades.length > 0) {
                            helper.showModal(component, event, result.especialidades);
                        } else {
                            LightningUtils.showToast("No hay datos", 'No se encontraron especialidades del efector en SaludSoft', {"type":"warning"});
                        }
                    }
                    else {
    ...
    showModal: function(component, event, especialidades) {
        $A.createComponent(
            "c:efectorEspecialidadesModal",
            {
