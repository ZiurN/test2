({
	goToApex: function (component, event, idAsociado) {
		...
		LightningUtils.callApex(
			...
			function(succeed, result, errors) {
				if(succeed) {
					if(!result.hasError){
						if(result.asociadoEmpleos.length > 0) {
							helper.showModal(component, event, result.asociadoEmpleos);
						} else {
							LightningUtils.showToast("No hay datos", 'No se encontraron empleos del afiliado en SaludSoft', {"type":"warning"});
						}
					} else {
	...
	showModal: function(component, event, empleos){
		$A.createComponent(
            "c:asociadoEmpleosModal",
            {
                "mydata" : empleos
            },
