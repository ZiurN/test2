({
	goToApex: function (component, event, codOS) {
		let helper = this;
		if(!codOS) {
			LightningUtils.showToast("Campo incompleto", 'El codigo de obra social no puede estar vacío');
			return;
		}
		component.set('v.isLoading', true);
		LightningUtils.callApex(
			component,
			"getObraSocialDomicilios",
				...
				if(succeed) {
					if(!result.hasError) {
						if(result.asociadoDomicilios.length > 0) {
							...
							LightningUtils.showToast("No hay datos", 'No se encontraron domicilios de la obra social en SaludSoft', {"type":"warning"});
						}
					}
					else {
